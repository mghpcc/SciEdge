...
podman run -d --network=host --tz=America/New_York -v $GIT_ROOT/var/www/ood/apps/sys/Flask:/var/www/ood/apps/sys/Flask:Z -v $GIT_ROOT/var/www/ood/apps/sys/Arctica:/var/www/ood/apps/sys/Arctica:Z -v $GIT_ROOT/var/www/ood/apps/sys/dashboard/config/locales:/var/www/ood/apps/sys/dashboard/config/locales:Z -v $SSL_CERT_ROOT_PATH:$SSL_CERT_ROOT_PATH:Z -v $GIT_ROOT/etc/ood:/etc/ood:Z -v $GIT_ROOT/etc/group:/etc/group:Z -v $GIT_ROOT/etc/passwd:/etc/passwd:Z -v /home:/home:Z --userns host --name ondemand_ctr ondemand_image /usr/sbin/init
